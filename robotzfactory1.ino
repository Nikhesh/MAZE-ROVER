...
// If only the left sensor is above the line, then turn LEFT
       if ((!digitalRead(L)) && (digitalRead(M)) && (digitalRead(R))) {
        digitalWrite (Lp,LOW);
        digitalWrite (Ln,LOW);
        digitalWrite (Rp,HIGH);
...
// If only the right sensor is above the line, then turn RIGHT
       if ((digitalRead(L)) && (digitalRead(M)) && (!digitalRead(R))) {
        digitalWrite (Lp,HIGH);
        digitalWrite (Ln,LOW);
        digitalWrite (Rp,LOW);
...
// What if two sensors say Middle & Right, are above the line? Well, essentially the bot should turn right, but it won't unless we specify any if statement for such condition.

// If Left & Middle sensors are above the line, then too turn RIGHT
       if ((!digitalRead(L)) && (!digitalRead(M)) && (digitalRead(R))) {
        digitalWrite (Lp,LOW);
        digitalWrite (Ln,LOW);
        digitalWrite (Rp,HIGH);
 ...
 // If Right & Middle sensors are above the line, then too turn LEFT
      if ((digitalRead(L)) && (!digitalRead(M)) && (!digitalRead(R))) {
        digitalWrite (Lp,HIGH);
        digitalWrite (Ln,LOW);
        digitalWrite (Rp,LOW);
